...
def counter(name: str, description: str | None = None, unit: str | None = None) -> Counter:
    ...
    if meter is not None:
        try:
            ...
            _logger.warning(
                "failed to create OTel counter %r, falling back to no-op", name, exc_info=True
            )  # pragma: no mutate
            return Counter(name)
    return Counter(name)
...
def gauge(name: str, description: str | None = None, unit: str | None = None) -> Gauge:
    ...
    if meter is not None:
        try:
            ...
            _logger.warning(
                "failed to create OTel gauge %r, falling back to no-op", name, exc_info=True
            )  # pragma: no mutate
            return Gauge(name)
    return Gauge(name)
...
def histogram(name: str, description: str | None = None, unit: str | None = None) -> Histogram:
    ...
    if meter is not None:
        try:
            ...
            _logger.warning(
                "failed to create OTel histogram %r, falling back to no-op", name, exc_info=True
            )  # pragma: no mutate
